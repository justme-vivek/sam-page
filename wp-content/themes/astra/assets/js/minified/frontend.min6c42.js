...
( () => {
    ...
    a = S.match(/(opera|chrome|safari|firefox|msie|trident(?=\/))\/?\s*(\d+)/i) || [],
    /trident/i.test(a[1]) ? t = /\brv[ :]+(\d+)/g.exec(S) || [] : "Chrome" === a[1] && null != (t = S.match(/\bOPR|Edge\/(\d+)/)) || (a = a[2] ? [a[1], a[2]] : [navigator.appName, navigator.appVersion, "-?"],
    null != (t = S.match(/version\/(\d+)/i)) && a.splice(1, 1, t[1]),
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    document.querySelectorAll(".menu-link .dropdown-menu-toggle").forEach(e => {
        ...
        e.addEventListener("keydown", function(e) {
            var t;
            "Enter" === e.key && (e = (e = this).closest(".menu-link"),
